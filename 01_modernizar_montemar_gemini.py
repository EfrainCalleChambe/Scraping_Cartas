...
Instalación:
    pip install google-genai beautifulsoup4 lxml pandas openpyxl regex

Configurar API key en CMD:
    set GEMINI_API_KEY=TU_API_KEY

Configurar API key en PowerShell:
    $env:GEMINI_API_KEY="TU_API_KEY"

Ejecución:
    python 01_modernizar_montemar_gemini.py

Notas:
    - Este script reemplaza el uso de OpenAI por Gemini.
    - Usa google-genai, el SDK actual de Google.
    - Procesa página por página.
    - Si una página ya existe, la salta para no repetir consumo.
"""
...
# Modelo Gemini. Puedes cambiarlo si en tu cuenta aparece otro disponible.
# Recomendado: gemini-2.5-flash o gemini-1.5-flash.
MODELO = "gemini-2.5-flash"

# Prueba primero con pocas páginas.
# Luego cambia a None para procesar todo.
MAX_PAGINAS = None

# Si ya existe el TXT moderno, se salta.
SALTAR_SI_EXISTE = True

# Pausa entre solicitudes. Si la API gratuita te limita, sube este valor.
ESPERA_SEGUNDOS = 6

# Reintentos ante errores temporales o límite de uso.
MAX_INTENTOS = 5

# Temperatura baja para reducir creatividad.
TEMPERATURE = 0.2

# Si la página es muy larga, se divide en partes.
# Gemini maneja bastante texto, pero dividir ayuda con límites gratuitos.
...
# ============================================================
# EXTRAER TEXTO ORIGINAL DESDE HTML
# ============================================================

def texto_diplomatico(node) -> str:
    """
    Extrae texto del HTML conservando la transcripción diplomática.
    Convierte superíndices HTML a Unicode superíndice.
    """
...
def dividir_texto(texto: str, max_chars: int = MAX_CARACTERES_POR_BLOQUE) -> list:
    """
    Divide por párrafos para evitar enviar bloques demasiado grandes.
    """
    ...
    for p in parrafos:
        ...
        if len(candidato) <= max_chars:
            ...
            if len(p) <= max_chars:
                ...
                # Corte de emergencia por longitud.
...
def crear_cliente_gemini():
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "Configura la clave antes de ejecutar:\n"
...
def extraer_texto_respuesta(response) -> str:
    """
    Intenta extraer texto de la respuesta Gemini de forma robusta.
    """
    if hasattr(response, "text") and response.text:
        return response.text.strip()

    # Respaldo por si el SDK devuelve partes.
    try:
        partes = []
...
def modernizar_bloque(client, texto_bloque: str, carta_id: str, pagina: int, num_bloque: int, total_bloques: int) -> str:
    ...
    for intento in range(1, MAX_INTENTOS + 1):
        try:
            ...
            # Esperas más largas para límites gratuitos.
            espera = ESPERA_SEGUNDOS * intento

            if "429" in msg or "quota" in msg.lower() or "rate" in msg.lower():
                espera = max(espera, 60 * intento)
...
def guardar_comparacion(carta_id: str, pagina: int, titulo: str, original: str, moderno: str, salida_path: Path):
...
def main():
    ...
    archivos = sorted(CARPETA_HTML_ORIGINAL.glob("*_page_*.html"))
    ...
    print(f"Páginas a modernizar: {len(archivos)}")
    ...
    for idx, path_html in enumerate(archivos, start=1):
        ...
        base_name = limpiar_nombre_archivo(f"{carta_id}_page_{pagina:03d}")
        ...
        if SALTAR_SI_EXISTE and txt_moderno_path.exists():
